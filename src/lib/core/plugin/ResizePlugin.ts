...
class ResizePlugin implements IPluginTempl {
  ...
  _watchDrag() {
    ...
    document.addEventListener('mouseup', this.eventHandler.onMouseUp)
  }
  ...
  destroy() {
    ...
    document.removeEventListener('mousemove', this.eventHandler.onDragging)
    document.removeEventListener('mouseup', this.eventHandler.onMouseUp)
    // 移除控制条 DOM，其上的 mousedown 监听随之释放
